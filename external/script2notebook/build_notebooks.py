...
def _inject_port_file_cell(notebook: Path, source: Path) -> bool:
    ...
    stripping, and ``include-book`` scanning that was previously done in
    Python.

    The ``.port`` file is generated by ``cert.pl`` during certification.
    If it does not exist (book not certified), this is a no-op.

    A marker (``provenance.portcullis``) in the injected cell's metadata
    prevents double-injection on re-runs.
    ...
    if not port_path.exists():
        log.debug("No .port file for %s — skipping portcullis injection", source)
        return False
    ...
    try:
        ...
        log.warning("Cannot inject portcullis cell for %s: %s", notebook, exc)
        return False

    # Don't inject twice — look for an existing cell tagged with
    # provenance.portcullis.
    for cell in nb.get("cells", []):
        if cell.get("metadata", {}).get("provenance", {}).get("portcullis"):
            return False
    ...
    # Prepend before existing cells.
    nb["cells"] = [eval_cell] + nb.get("cells", [])
